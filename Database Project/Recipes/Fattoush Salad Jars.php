...
$recipe_id = 7; // Use the correct recipe ID
...
<html lang="en" dir="ltr">
    ...
    const recipeId = 7; // Correct recipe ID
    ...
    if (isRed) {
        ...
        <?php
        $sql = "SELECT description FROM recipes WHERE recipe_id = 7";
            $result = $conn->query($sql);
            if ($result && $result->num_rows > 0) {
                // Output data
            ...
            $sql = "SELECT instructions FROM recipes WHERE recipe_id = 7";
            $result = $conn->query($sql);
